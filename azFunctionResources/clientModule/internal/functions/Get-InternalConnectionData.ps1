...
#>
	...
	process
	{
		try { $uri = '{0}{1}' -f (Get-PSFConfigValue -FullName 'SPOTools.Client.Uri' -NotNull), $FunctionName }
		catch { $PSCmdlet.ThrowTerminatingError($_) }
		$header = @{ }
		
		#region Authentication
		$unprotectedToken = Get-PSFConfigValue -FullName 'SPOTools.Client.UnprotectedToken'
		$protectedToken = Get-PSFConfigValue -FullName 'SPOTools.Client.ProtectedToken'
