...
$parentModule = 'SPOTools'
if (-not $ModuleName) { $ModuleName = 'SPOTools.Client' }
...
$publishRoot = Join-Path -Path $WorkingDirectory -ChildPath 'publish\SPOTools'
...
$originalManifestData = Import-PowerShellDataFile -Path "$publishRoot\SPOTools.psd1"
$prereqHash = @{
